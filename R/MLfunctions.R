...
maxLRatio_phi_BruteForce = function(n1,
  ...
  if (returnRaw == TRUE) {
    return(list(plain = data.table(n1=n1,n2=n2, log10_alpha = optimSol$minlevels[1], beta = optimSol$minlevels[2], LLratio = log(-optimSol$minfun)), raw = optimSol))
  } else {
    return(data.table(n1=n1,n2=n2, log10_alpha = optimSol$minlevels[1], beta = optimSol$minlevels[2], LLratio = log(-optimSol$minfun)))
  }
}
...
LratioCalcPhi = function(n1,
  ...
  LLvec_sel <- sum(log(getProbVectorPhiS(phitable = phiSTable, n1 = n1, minN1 = minN1,k1 = sub_partition_k1_counts,k2 = sub_partition_k2_counts,sub_alphaD_idx = sub_alphaD_idx)))
  LLvec_neu <- sum(log(getProbVectorPhiN(phitable = phiNTable, n1 = n1, minN1 = minN1,k1 = sub_partition_k1_counts,k2 = sub_partition_k2_counts)))
  if(monomorphic==TRUE){
  ...
  LLratio <- exp(sum(LLvec_sel) - sum(LLvec_neu))
